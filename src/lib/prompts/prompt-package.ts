...
5. 可以使用 Yes-but 或 No-and 形成升级，但只在符合本书题材和章节位置时使用，不规定比例
6. 允许安排安静章、关系章或缓冲章，数量服从故事节奏
7. 只记录真正会影响后续阅读的伏笔，不为凑数量添加线索；能确认的才标注埋设、暗示、揭示位置
8. 相邻章节分别标明已完成事件、本章新增推进和下一章边界，禁止换措辞重复同一冲突
9. 终局揭示、角色生死、救援结果和核心底牌只能在规划位置兑现`,
  },
  {
...
- 关系、组织和职业只引用项目中真实存在的名称；无法确认的关联留空，不把推断包装成既有事实。
...
5. 每一条规则都应该是某个情节的孵化器
6. 矛盾必须是结构性的
7. 时间尺度、空间范围、社会结构和规则复杂度服从题材；现代/校园/职场题材不套史诗尺度
8. 明确时间与社会状态、空间与生活方式、感官氛围、规则及违反后果，四者互相支撑`,
  },
...
- 默认最小改动：只处理有问题的表达，不偷改剧情、关系、视角和人物动机`,
  },
